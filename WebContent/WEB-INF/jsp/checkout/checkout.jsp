...
<div class="inner-page padd">	
		...
		<div class="container">	
			<div class="row">
				...
				<%-- <p><fmt:message key="prodsEnComanda"/>: ${fn:length(comandaActual.liniesComanda)}</p> --%>
						<table class="table table-stripped tableEwok">
								<tr>
							...
							<c:forEach items="${comandaActual.liniesComanda}" var="liniaComanda">
								<tr>
									<td><img class="imgCheck" alt="img_wok" src="img?accioFoto=get&idProd=${liniaComanda.producte.id}"></td>									
									<td>${liniaComanda.producte.nom}</td>
									<td>${liniaComanda.producte.descripcio}</td>
									<td><input type="number" step="1" value="${liniaComanda.quantitat}"/> <i class="fa fa-pencil"/></td>
									<td>${liniaComanda.preuVenda * liniaComanda.quantitat} €</td>
									<td><i title="eliminar línia" class="fa fa-times alert-danger tancaOnclick"></i></td>									
								</tr>
							</c:forEach>
							<c:forEach items="${comandaActual.liniesComanda}" var="liniaComanda">
								<tr>
									<td><img class="imgCheck" alt="img_wok" src="https://www.tuelsalvador.com/components/com_virtuemart/shop_image/product/Coca_Cola_Cl__si_4cd9628c5e4a5.gif"></td>									
									<td>${liniaComanda.producte.nom}</td>
									<td>${liniaComanda.producte.descripcio}</td>
									<td><input type="number" step="1" value="${liniaComanda.quantitat}"/> <i class="fa fa-pencil"/></td>
									<td>${liniaComanda.preuVenda * liniaComanda.quantitat} €</td>
					...
					<p><fmt:message key="anar"/> <a href="index.html">Home</a> | <a href="staff"><fmt:message key="areaPrivada"/></a></p>					
	...
	<!-- Script per tancar els alerts -->
		<script src = "jseWok/tancarLinies.js"></script>		
	</body>	
